...
# ============================================================
# 4. ✅ 新增核心组件：MDTA & ShiftedWindowMHSA
# ============================================================

class MDTA(nn.Module):
    """
    Multi-Dconv Head Transposed Attention (MDTA)
    用于 L2 (Low-Freq) 的 LL 分量：在通道维度计算 Attention，捕捉全局结构。
    """
    ...
    def forward(self, x):
        ...
        # 计算通道 Attention Map: (B, Heads, C, C)
...
class ShiftedWindowMHSA(nn.Module):
    """
    Shifted Window MHSA
    用于 L1 (High-Freq) 的 High 分量：通过循环移位解决窗口边界问题。
    """
...
# ============================================================
# 5. FSAS 混合版 (Integrated MDTA & Shifted Window)
# ============================================================
class FSAS_Hybrid(nn.Module):
    def __init__(self, dim, bias, mode='local_struct'):
        ...
        if mode == 'local_struct': # L2: 低频结构层
            # ============================================================
            # 策略：LL (Low Freq) -> MDTA (全局结构校正)
            #       High (High Freq) -> CNN (辅助)
            # ============================================================
            # LL 分量使用 MDTA (Global Structure)
        ...
        elif mode == 'high_freq_attention': # L1: 高频细节层
            # ============================================================
            # 策略：High (High Freq) -> Shifted Window MHSA (细节精修)
            #       LL (Low Freq) -> CNN (辅助)
            # ============================================================
            # High 分量使用 Shifted Window (Window Texture)
            # shift_size 设为 window_size // 2
    ...
    def forward(self, x):
        ...
        if self.mode == 'local_struct': # L2
            # Main: LL 经过 MDTA
            # MDTA 输入 (B,C,H,W) -> 输出 (B,C,H,W)
        ...
        elif self.mode == 'high_freq_attention': # L1
            # Main: High 经过 Shifted Window
            high_norm = self.norm_main(high)
            # ShiftedWindowMHSA 内部已处理 Padding 和 Permute
...
# 7. EDFFN (混合自适应版)
# ============================================================
class EDFFNModed_Hybrid(nn.Module):
...
class SimpleEncoderBlock(nn.Module):
    def __init__(self, dim, ffn_expansion_factor=3.0, bias=False):
        ...
        self.ffn = EDFFNModed_Hybrid(dim, ffn_expansion_factor, bias, mode='full', patch_size=8)
...
# ============================================================
# 9. Decoder Block (Updated to use FSAS_Hybrid)
# ============================================================
class FFTformerBlock(nn.Module):
    def __init__(self, dim, ffn_expansion_factor=3.0, bias=False, ffn_mode='full',
        ...
        # ✅ 使用升级版 FSAS_Hybrid
        self.attn = FSAS_Hybrid(dim, bias, mode=mode)
        self.norm2 = LayerNorm(dim)
        self.ffn = EDFFNModed_Hybrid(dim, ffn_expansion_factor, bias, ffn_mode, 8,
...
class EAMaIR_ZeroWaste(nn.Module):
...
class EVS(nn.Module):
    def __init__(self, dim, ffn_expansion_factor=3.0, bias=False, att=True,
        ...
        super().__init__()
        self.norm1 = LayerNorm(dim)
        self.attn = EAMaIR_ZeroWaste(dim=dim, overlap_ratio=overlap_ratio) if att else nn.Identity()
        self.norm2 = LayerNorm(dim)
        self.ffn = EDFFNModed_Hybrid(dim, ffn_expansion_factor, bias, ffn_mode)
...
class EVSSM(nn.Module):
    def __init__(self, 
        ...
        self.encoder_level3 = nn.Sequential(*[
            EVS(dim*4, ffn_expansion_factor, bias, att=True, patch=96, ffn_mode='full',
        ...
        # L2: MDTA + Low-Freq Boost
        self.fuse2 = ChannelAttentionFuse(dim * 2, reduction=16, bias=bias)
        self.decoder_level2 = nn.Sequential(*[
            FFTformerBlock(dim*2, ffn_expansion_factor, bias, ffn_mode='low_emphasis',
                           radius_low=radius_low, mode='local_struct')
            for _ in range(num_blocks[1])
        ])
        self.up2_1 = Upsample(dim * 2)

        # L1: Shifted Window + High-Freq Boost
        self.fuse1 = ChannelAttentionFuse(dim, reduction=8, bias=bias)
        self.decoder_level1 = nn.Sequential(*[
            FFTformerBlock(dim, ffn_expansion_factor, bias, ffn_mode='high_emphasis',
...
if __name__ == '__main__':
    torch.manual_seed(0)
    print("Testing Final Hybrid Model...")
    model = EVSSM(dim=48, num_blocks=(2, 2, 2)) # Reduced for quick test
    x = torch.randn(1, 3, 128, 128)
    y = model(x)
    print(f"Input: {x.shape}, Output: {y.shape}")
    print("Validation Complete. Dimensions are correct.")