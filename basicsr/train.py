...
def parse_options(is_train=True):
    ...
    parser.add_argument('--input_path', type=str, required=False, help='For single image inference only.')
    parser.add_argument('--output_path', type=str, required=False, help='For single image inference only.')
...
def create_train_val_dataloader(opt, logger):
    ...
    for phase, dataset_opt in opt['datasets'].items():
        if phase == 'train':
            ...
            num_iter_per_epoch = math.ceil(len(train_set) * dataset_enlarge_ratio /
...
def main():
    ...
    if opt['path'].get('resume_state'):
        ...
        print(f"Load resume_state from YAML: {opt['path']['resume_state']}")
    else:
        resume_state = None
        print("No resume_state specified in YAML. Start fresh training.")
    # --------------------------------------------------
    ...
    if opt['rank'] == 0:
        try:
            params_m = sum(p.numel() for p in model.net_g.parameters() if p.requires_grad) / 1e6
            logger.info(f'\n\n[Network G] Total Trainable Parameters: {params_m:.2f} M\n')
        except Exception as e:
            logger.warning(f"Could not calculate parameter count: {e}")
